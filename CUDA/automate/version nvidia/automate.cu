...
__global__ void blend_kernel( float *etatS, bool dstOut ) {
    ...
    /*if(c == M) { 			//si la cellule est malade elle devient saine
    	etatS[offset] = 0;
    } else if (c == 0) {	//si elle est saine, (#voisins non sains)/K
    	etatS[offset] = (t + l + r + b) / K;
    } else if (c != 0 && c != M) {
    	etatS[offset] = ((t + l + c + r + b) / 5) + G;
    } else if(c > M) {
    	etatS[offset] = M;
    }*/
    etatS[offset] = c + SPEED * (t + b + r + l - 4 * c);
}
...
//                                    size_t texOffset )
__global__ void copy_const_kernel( float *iptr ) {	//c'est le color
    // map from threadIdx/BlockIdx to pixel position
    int x = threadIdx.x + blockIdx.x * blockDim.x;
    int y = threadIdx.y + blockIdx.y * blockDim.y;
    int offset = x + y * blockDim.x * gridDim.x;
...
int main( void ) {
    ...
    srand(time(NULL));
    float *temp = (float*)malloc( imageSize );	//tableau des températures
    for (int i=0; i<DIM*DIM; i++) {
        temp[i] = 0;
    }
    //point qui change pas -> nécéssaire pour init le tableau temp a 0
